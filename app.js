...
function renderTrends(){
  $('#trendList').innerHTML=DATA.trends.map(t=>`<div class="trend-item"><div class="trend-icon">${t.icon}</div><div><h3>${t.title}</h3><p>${t.text}</p></div></div>`).join('');
}
function renderSources(){
  $('#sources').innerHTML=DATA.sources.map(s=>`<a class="source-link" href="${s.url}" target="_blank" rel="noopener"><span>${s.label}</span><span>abrir ↗</span></a>`).join('');
}
...
async function loadData(cacheBust=false){
  try{
    const res=await fetch(`data/dashboard.json${cacheBust?'?t='+Date.now():''}`);
    DATA=await res.json();
  }catch(e){
    console.error(e); alert('No pude leer data/dashboard.json. Abre la web desde un servidor local o GitHub Pages.'); return;
  }
